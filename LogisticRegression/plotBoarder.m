...
% model3 attributes only include parameter of multinomial logistic regression:
%     model3.w: [3x3 double]
...
prob2 = zeros(n,3);
prob2(:,1) = 1/sqrt(det(model2.S1))*exp(-0.5*sum(bsxfun(@minus, xy, model2.m1)*inv(model2.S1).*bsxfun(@minus, xy, model2.m1),2))*model2.pi(1);
prob2(:,2) = 1/sqrt(det(model2.S2))*exp(-0.5*sum(bsxfun(@minus, xy, model2.m2)*inv(model2.S2).*bsxfun(@minus, xy, model2.m2),2))*model2.pi(2);
prob2(:,3) = 1/sqrt(det(model2.S3))*exp(-0.5*sum(bsxfun(@minus, xy, model2.m3)*inv(model2.S3).*bsxfun(@minus, xy, model2.m3),2))*model2.pi(3);
